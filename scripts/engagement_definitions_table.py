...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Extract 'Engagement Citation', 'Engagement Concept', and 'BibTex Key' "
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/engagement_definitions.csv"),
        help="Path to the output .csv file (default: data/engagement_definitions.csv).",
    )
    args = parser.parse_args(argv)

    desired_columns = ['Engagement Citation', 'Engagement Concept', 'BibTex Key']
